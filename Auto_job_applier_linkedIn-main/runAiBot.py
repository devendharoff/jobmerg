...
def is_logged_in_LN() -> bool:
    ...
    * Returns: `True` if user is logged-in or `False` if not
    ...
    if "chrome-error" in current_url or "dnserror" in current_url:
        return False
...
def login_LN() -> None:
    ...
    * If both failed, asks user to login manually
    ...
    if username == "username@example.com" and password == "example_password":
        print_lg("Opening Chrome for LinkedIn Login. Please log in to your account directly in the opened Chrome browser window!")
...
def apply_filters() -> None:
    ...
    try:
        ...
        if pause_after_filters:
            print_lg("Bypassing pause_after_filters alert dialog inside background execution.")
            pause_after_filters = False

    except Exception as e:
        print_lg("Setting the preferences failed!")
        print_lg(f"Bypassing filter error alert dialog. Filter Error Details: {e}")
        # print_lg(e)
...
def get_job_main_details(job: WebElement, blacklisted_companies: set, rejected_jobs: set) -> tuple[str, str, str, str, str, bool]:
    ...
    * skip: A boolean flag to skip this job
    ...
    other_details = job.find_element(By.CLASS_NAME, 'artdeco-entity-lockup__subtitle').text.strip()
    ...
    if '(' in work_location and ')' in work_location:
        start_idx = work_location.rfind('(')
        end_idx = work_location.rfind(')')
        if start_idx < end_idx:
            work_style = work_location[start_idx+1:end_idx].strip()
            work_location = work_location[:start_idx].strip()
...
def failed_job(job_id: str, job_link: str, resume: str, date_listed, error: str, exception: Exception, application_link: str, screenshot_name: str) -> None:
    ...
    try:
        ...
        print_lg("WARNING: Failed to update the excel of failed jobs! Check file lock/permissions.")
...
def save_live_screenshot(driver: WebDriver) -> None:
    try:
        os.makedirs(logs_folder_path, exist_ok=True)
        driver.save_screenshot(os.path.join(logs_folder_path, "live.png"))
    except Exception:
        pass
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'], 
    ...
    try:
        ...
        print_lg("WARNING: Failed to update the excel of applied jobs! Check file lock/permissions.")
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                        ...
                        try: 
                            try:
                                ...
                                while next_button:
                                    ...
                                    if next_counter >= 15: 
                                        if pause_at_failed_question:
                                            ...
                                            print_lg("Couldn't answer one or more questions. Skipping/Continuing.")
                                            next_counter = 1
                                            continue
                                ...
                                if errored != "stuck" and cur_pause_before_submit:
                                     print_lg("Bypassing pause_before_submit alert dialog inside background execution. Automatically submitting.")
                    ...
                    if easy_applied_count >= total_applications_limit:
                        print_lg(f"\n###############  Total application limit of {total_applications_limit} reached! Stopping bot execution strictly.  ###############\n")
...
def main() -> None:
    ...
    try:
        ...
        if not os.path.exists(default_resume_path):
            print_lg('Your default resume "{}" is missing! The bot will continue using your previously uploaded resume in LinkedIn.'.format(default_resume_path))
            useNewResume = False
