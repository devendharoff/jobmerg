...
def manual_login_retry(is_logged_in: callable, limit: int = 2) -> None:
    '''
    Function to check for manual login by polling the state
    '''
    print_lg("Waiting up to 120 seconds for user to log into LinkedIn directly in the Chrome browser window...")
    for sec in range(120):
        if is_logged_in():
            print_lg("LinkedIn login session detected! Proceeding to job search...")
            return
        try:
            from runAiBot import driver
            import os
            os.makedirs("logs", exist_ok=True)
            driver.save_screenshot("logs/live.png")
        except Exception:
            pass
        sleep(1)
    print_lg("Timeout waiting for LinkedIn login. Proceeding anyway.")
